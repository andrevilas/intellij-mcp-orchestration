...
1. Regenerar o dataset FinOps com `python scripts/generate_finops_fixtures.py --seed-db --db-path <caminho/console.db>` (reescreve fixtures + popula `telemetry_events`) e exportar `CONSOLE_MCP_DB_PATH=<caminho/console.db>` antes de repetir `GET /api/v1/telemetry/finops/sprints?start=2025-10-08T00:00:00Z&end=2025-10-21T23:59:59Z&window_days=7`, `GET /api/v1/telemetry/finops/pull-requests?start=2025-10-15T00:00:00Z&end=2025-10-21T23:59:59Z&window_days=7` e `GET /api/v1/telemetry/export` (CSV padrão + `format=html`). Salve as respostas em `docs/evidence/<DATA>/` como prova de que os dashboards refletem dados reais nas janelas recomendadas.【F:scripts/generate_finops_fixtures.py†L1-L391】【88a770†L1-L28】【88a770†L28-L43】
