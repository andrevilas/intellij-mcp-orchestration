...
def test_registry_loads_multiple_agents(settings, create_agent) -> None:
    create_agent("alpha")
    create_agent("beta")
...
def test_registry_lazy_instantiation(settings, create_agent) -> None:
    create_agent("delta")
    registry = AgentRegistry(root=settings.agents_root, logger=logging.getLogger("test.registry"))
    ...
    asyncio.run(registry.invoke("delta", {"message": "hello"}))

    assert record.instance is not None
    assert getattr(record.instance, "invocations") == 1


def test_registry_validates_payload_shape(settings, create_agent) -> None:
    create_agent("gamma")
    registry = AgentRegistry(root=settings.agents_root, logger=logging.getLogger("test.registry"))

    with pytest.raises(ValidationError):
        asyncio.run(registry.invoke("gamma", ["invalid", "payload"]))  # type: ignore[arg-type]
